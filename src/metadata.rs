...
pub fn get_meta_value(meta_path: String, category: &str) -> String {
    if !matches!(
        ...
        panic!("Invalid metadata category being retrieved: {category}")
    }
    ...
    fs::read_to_string(&meta_path)
        .expect("Failed to retrieve project metadata")
        // Seperate the lines
        .split('\n')
        ...
        .unwrap_or_else(|| panic!(
            "Failed to retrieve metadata value from category \"{category}\" in file \"{meta_path}\""
        ))
